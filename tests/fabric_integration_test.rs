...
async fn test_fabric_analyzer_detects_nondeterminism() {
    ...
    let result = analyzer
        .analyze_chaincode(path)
        .await
        .expect("Analysis failed");

    // Should detect nondeterminism issues
    let nondeterminism_findings: Vec<_> = result
        .findings
        .iter()
    ...
    assert!(
        !nondeterminism_findings.is_empty(),
        "Should detect nondeterminism issues"
    );
    assert!(nondeterminism_findings
        .iter()
        .any(|f| f.id == "FABRIC-ND-001"));
}
...
async fn test_fabric_analyzer_calculates_scores() {
    ...
    assert!(
        result.fabric_best_practices_score >= 0.0 && result.fabric_best_practices_score <= 100.0
    );
}
