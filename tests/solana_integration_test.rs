...
async fn test_solana_analyzer_detects_arithmetic_issues() {
    ...
    let result = analyzer.analyze_program(path).await.expect("Analysis failed");
    
    // Should detect arithmetic issues
    let arithmetic_findings: Vec<_> = result.findings.iter()
        .filter(|f| f.category.contains("Arithmetic"))
        .collect();
    
    assert!(!arithmetic_findings.is_empty(), "Should detect arithmetic issues");
...
async fn test_solana_analyzer_detects_account_validation_issues() {
    ...
    let account_findings: Vec<_> = result.findings.iter()
        .filter(|f| f.category.contains("AccountValidation"))
        .collect();
    
    assert!(!account_findings.is_empty(), "Should detect account validation issues");
}
...
async fn test_solana_analyzer_detects_signer_check_issues() {
    ...
    let signer_findings: Vec<_> = result.findings.iter()
        .filter(|f| f.category.contains("SignerCheck"))
        .collect();
    
    assert!(!signer_findings.is_empty(), "Should detect signer check issues");
}
