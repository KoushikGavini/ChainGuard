...
impl FabricAnalyzer {
    ...
    fn check_endorsement_policy(
        ...
        if !has_creator_check && !has_msp_id_check {
            findings.push(Finding {
                ...
                description: "Chaincode does not validate transaction creator or MSP ID. \
                    This could allow unauthorized organizations to invoke transactions."
                    .to_string(),
                file: "".to_string(),
                line: 1,
                column: 1,
                code_snippet: None,
                remediation: Some(
                    "Implement proper access control using stub.GetCreator() and validate MSP IDs"
                        .to_string(),
                ),
                references: vec![],
                ai_consensus: None,
    ...
    fn check_private_data_security(
        &self,
        content: &str,
        tree: &tree_sitter::Tree,
    ) -> Result<Vec<Finding>> {
                ...
                for i in get_line..std::cmp::min(get_line + 10, lines.len()) {
                    if put_regex.is_match(lines[i - 1]) {
                        findings.push(Finding {
                            ...
                            description: "Private data appears to be written to public state. \
                                This could expose confidential information to all channel members."
                                .to_string(),
                            file: "".to_string(),
                            line: get_line,
                            column: 1,
                            ...
                            remediation: Some(
                                "Use PutPrivateData() to store private data, not PutState()"
                                    .to_string(),
                            ),
                            references: vec![],
                            ai_consensus: None,
    ...
    fn check_mvcc_compliance(
        ...
        let read_patterns = vec![r"GetState\s*\(", r"GetStateByRange\s*\("];
    ...
    fn check_dos_vulnerabilities(
        ...
        let loop_patterns = vec![r"for\s+range\s+", r"for\s*\{", r"for\s+.*;.*;.*\{"];
        ...
        let large_data_patterns = vec![r"GetStateByRange\s*\([^)]*\)", r"GetHistoryForKey\s*\("];
    ...
    fn check_channel_isolation(
        ...
        let channel_patterns = vec![r"GetChannelID\s*\(", r"channel\s*:="];
        ...
        if has_channel_check {
            findings.push(Finding {
                ...
                description:
                    ...
                    and no cross-channel data leakage."
                        .to_string(),
                file: "".to_string(),
                line: 1,
                column: 1,
