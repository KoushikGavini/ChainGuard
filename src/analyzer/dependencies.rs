...
impl DependencyAnalyzer {
    pub fn new() -> Self {
        ...
        let external_call_patterns = vec![
            ...
            ExternalCallPattern {
                ...
                severity: Severity::Critical,
                description: "External database connections are not allowed in chaincode".to_string(),
            },
            ExternalCallPattern {
                name: "file_operations".to_string(),
                regex: Regex::new(r"(os\.(Open|Create|Remove)|ioutil\.(ReadFile|WriteFile))").unwrap(),
                severity: Severity::Critical,
    ...
    fn analyze_imports(&self, imports: &HashSet<String>, path: &Path, findings: &mut Vec<Finding>) {
        let unsafe_packages = vec![
            ...
            ("reflect", "Reflection can lead to non-deterministic behavior"),
            ("plugin", "Dynamic plugin loading is not allowed in chaincode"),
            ("cgo", "C bindings are not supported in chaincode"),
            ("runtime/debug", "Debug package can expose sensitive information"),
        ];
            ...
            if !import.starts_with("github.com/hyperledger/fabric") && 
               ...
               !is_safe_third_party(import) {
                findings.push(Finding {
                    ...
                    description: format!("External dependency '{}' should be carefully reviewed", import),
                    file: path.display().to_string(),
                    line: 0,
                    column: 0,
                    code_snippet: None,
    ...
    fn check_unsafe_packages(&self, content: &str, path: &Path, findings: &mut Vec<Finding>) {
        ...
        let unsafe_patterns = vec![
            ...
            (r"reflect\.ValueOf.*\.Pointer\(\)", "Unsafe reflection usage"),
...
fn is_standard_library(import: &str) -> bool {
    let std_packages = vec![
        "fmt", "strings", "strconv", "bytes", "errors", "encoding/json",
        "encoding/base64", "encoding/hex", "sort", "container/list",
        "math", "math/big", "time", "unicode", "unicode/utf8"
    ];
    
    std_packages.iter().any(|&pkg| import == pkg || import.starts_with(&format!("{}/", pkg)))
}

fn is_safe_third_party(import: &str) -> bool {
    // Known safe third-party packages commonly used in chaincode
    let safe_packages = vec![
