...
impl Validator {
    ...
    pub async fn validate(&self, content: &str, path: &Path) -> Result<Vec<Finding>> {
        ...
        let dep_findings = self
            .dep_validator
            .validate(content, path, &self.trusted_packages)
            .await?;
            ...
            && !import_path.starts_with("github.com/hyperledger/")
        {
            ...
            if self.is_suspicious_import(import_path) {
                return Some(Finding {
                    ...
                    code_snippet: None,
                    remediation: Some(
                        "Verify the import path is correct and from a trusted source".to_string(),
                    ),
                    references: vec![],
                    ai_consensus: None,
                });
    ...
    fn is_suspicious_import(&self, import_path: &str) -> bool {
        ...
        let suspicious_patterns = vec![
            (
                "hyperledger",
                vec!["hyperledge", "hyperleger", "hyperledgerr", "hyperldger"],
            ),
            ("fabric", vec!["fabrik", "fabic", "fabricc", "frabiс"]), // Note: last 'c' is Cyrillic
            (
                "chaincode",
                vec!["chaincode", "chaincde", "chainocde", "chaincоde"],
            ), // Note: 'o' is Cyrillic
        ];
