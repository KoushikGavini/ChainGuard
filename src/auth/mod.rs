use crate::{Result, ChainGuardError};
...
impl AuthManager {
    ...
    pub async fn load(&mut self) -> Result<()> {
        if self.config_path.exists() {
            ...
            let config: AuthConfig = toml::from_str(&content)
                .map_err(|e| ChainGuardError::Config(format!("Failed to parse auth config: {}", e)))?;
    ...
    pub async fn save(&self) -> Result<()> {
        ...
        let content = toml::to_string_pretty(&config)
            .map_err(|e| ChainGuardError::Config(format!("Failed to serialize auth config: {}", e)))?;
    ...
    pub async fn set_api_key(&mut self, service: &str, api_key: &str) -> Result<()> {
        ...
        let valid_services = vec!["chatgpt", "claude", "gemini", "openai", "anthropic", "google"];
        
        if !valid_services.contains(&service_lower.as_str()) {
            return Err(ChainGuardError::Auth(
                format!("Unknown service: {}. Valid services are: {:?}", service, valid_services)
            ));
        }
    ...
    pub async fn remove_api_key(&mut self, service: &str) -> Result<()> {
        ...
        if self.credentials.remove(&service_lower).is_none() {
            return Err(ChainGuardError::Auth(
                format!("No credentials found for service: {}", service)
            ));
        }
    ...
    pub async fn get_api_key(&mut self, service: &str) -> Result<String> {
        ...
        self.credentials
            ...
            .ok_or_else(|| ChainGuardError::Auth(
                format!("No API key found for service: {}", service)
            ))
    }
    ...
    pub async fn test_connection(&mut self, service: &str) -> Result<()> {
        ...
        match service.to_lowercase().as_str() {
            ...
            _ => Err(ChainGuardError::Auth(format!("Unknown service: {}", service))),
        }
    }
    ...
    async fn test_openai_connection(&self, api_key: &str) -> Result<()> {
        ...
        if response.status().is_success() {
            ...
            Err(ChainGuardError::Auth(
                format!("OpenAI API test failed: {}", response.status())
            ))
        }
    }
    ...
    async fn test_anthropic_connection(&self, api_key: &str) -> Result<()> {
        ...
        if response.status().as_u16() == 405 || response.status().is_success() {
            ...
            Err(ChainGuardError::Auth(
                format!("Anthropic API test failed: {}", response.status())
            ))
        }
    }
    ...
    async fn test_google_connection(&self, api_key: &str) -> Result<()> {
        ...
        if response.status().is_success() {
            ...
            Err(ChainGuardError::Auth(
                format!("Google AI API test failed: {}", response.status())
            ))
        }
    }
