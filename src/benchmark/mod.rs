...
impl Benchmarker {
    ...
    pub async fn analyze_throughput(&self, path: &Path) -> Result<ThroughputResult> {
        ...
        let db_ops = content.matches("GetState").count() + 
                    content.matches("PutState").count();
        if db_ops > 10 {
    ...
    pub async fn analyze_storage(&self, path: &Path) -> Result<StorageResult> {
        ...
        let json_count = content.matches("json.Marshal").count() + 
                        content.matches("JSON.stringify").count();
        if json_count > 5 {
            efficiency -= 10.0;
        }
    ...
    pub async fn analyze_consensus(&self, path: &Path) -> Result<ConsensusResult> {
        ...
        if self.fabric_enabled {
            ...
            let endorsement_time = 10.0 + (get_state_count as f32 * 2.0) + (put_state_count as f32 * 3.0);
            
            // Check for private data
            if content.contains("GetPrivateData") || content.contains("PutPrivateData") {
                overhead_ms += 20.0;
                optimization_suggestions.push(
                    "Private data operations increase consensus overhead".to_string()
                );
            }
            ...
            if content.contains("InvokeChaincode") {
                overhead_ms += 30.0;
                optimization_suggestions.push(
                    "Cross-chaincode calls significantly impact consensus time".to_string()
                );
            }
            ...
            if get_state_count > 5 {
                optimization_suggestions.push(
                    "Consider batching GetState operations".to_string()
                );
            }
            
            if put_state_count > 3 {
                optimization_suggestions.push(
                    "Consider combining multiple PutState operations".to_string()
                );
            }
            ...
            Ok(ConsensusResult {
                ...
                commit_time_ms: 10.0,
                optimization_suggestions: vec!["Platform-specific analysis not available".to_string()],
            })
        }
    }
