...
impl LLMManager {
    ...
    pub async fn analyze_with_consensus(
        ...
        // Create tasks inside a scope to limit provider lock lifetime
        {
            ...
            let provider_list: Vec<(String, Arc<dyn LLMProvider>)> = providers
                .iter()
            ...
            for (name, provider) in provider_list {
                ...
                let task =
                    tokio::spawn(
                        async move { (name, provider.analyze_code(&code, &context).await) },
                    );
        ...
        for (provider_name, result) in &results {
            for finding in &result.findings {
                ...
                let consensus =
                    consensus_findings
                        .entry(key)
                        .or_insert_with(|| ConsensusFindings {
    ...
    pub async fn validate_ai_generated_with_consensus(
        ...
        // Create tasks inside a scope to limit provider lock lifetime
        {
            ...
            for provider in provider_list {
                ...
                let task = tokio::spawn(async move { provider.validate_ai_generated(&code).await });
    ...
    fn aggregate_validations(
        &self,
        validations: Vec<AIGeneratedValidation>,
    ) -> Result<AIGeneratedValidation> {
        let count = validations.len() as f32;

        let avg_confidence = validations.iter().map(|v| v.confidence).sum::<f32>() / count;

        let is_ai_generated =
            validations.iter().filter(|v| v.is_ai_generated).count() as f32 / count >= 0.5;
        ...
        let avg_quality = validations.iter().map(|v| v.quality_score).sum::<f32>() / count;
