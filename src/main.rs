...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Init { config, platform, examples, force } => {
        ...
        Commands::History { limit, filter, detailed, export } => {
...
async fn analyze_command(
    ...
    // Print summary
    let total_findings = results.iter()
...
async fn init_command(config_path: PathBuf, platform: Option<String>, examples: bool, force: bool) -> Result<()> {
    println!("{}", style("⚙️  Configuration Initialization").bold().cyan());
    println!("{}", style("━".repeat(50)).dim());
    
    if config_path.exists() && !force {
        eprintln!("{}", style("Configuration file already exists. Use --force to overwrite.").red());
        return Ok(());
    }
...
async fn history_command(limit: usize, filter: Option<String>, detailed: bool, export: Option<PathBuf>, format: OutputFormat) -> Result<()> {
...
async fn scan_command(
    ...
    let findings_count = results.iter()
        .flat_map(|r| &r.findings)
    ...
    if !quiet {
        ...
        println!("{}", style("━".repeat(50)).dim());
        println!("Total vulnerabilities found: {}", style(findings_count).bold());
    }
...
async fn audit_command(
    ...
    if !quiet {
        ...
        println!("{}", style("━".repeat(50)).dim());
        println!("Compliance Score: {}%", style(results.compliance_score).bold());
...
async fn optimize_command(
    ...
    if !quiet {
        ...
        println!("Estimated performance gain: {}%", 
